...
public class AsyncLock
{
    ...
    // whatever.
    private readonly SemaphoreSlim _retry = new(0, 1);
    private const long UnlockedId = 0; // "owning" task id when unlocked
    private long _owningId = UnlockedId;
    private int _owningThreadId = (int)UnlockedId;
    ...
    private readonly struct InnerLock : IDisposable
    {
        ...
        public void Dispose()
        {
            ...
            try
            {
                ...
                if (_parent._reentrances == 0)
                {
                    ...
                    _parent._owningId = UnlockedId;
                    _parent._owningThreadId = (int)UnlockedId;
                }
    ...
    public Task<IDisposable> LockAsync(CancellationToken ct = default)
    {
        ...
        _asyncId.Value = Interlocked.Increment(ref AsyncStackCounter);
